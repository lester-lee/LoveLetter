...
public void takeTurn() {
        ...
        if (p.type == "human") {
                System.out.println(this);            // print the table
        }
...
private Card askPlayer(Player p) {
        Card playedCard;
        if (p.type == "human") {
                System.out.println("Which card would Player #" + p.number + " like to play? Your options are:");
                p.printOptions();
...
private void takeAction(Card c) {
        ...
        //System.out.println("Player# " + curP.number + " : " + c.type);
        switch (c.type) {
        case 1:
                guardAction(curP);
                break;
        ...
        case 3:
                ...
                break;
        case 4:
                //System.out.println("Player #" + curP.number + " is immune until their next turn.");
                break;
        case 5:
                princeAction(curP);
                break;
        case 6:
                ...
                break;
        case 8:
                //System.out.println("Player #" + curP.number + " discarded the Princess!.");
...
private Player validCheck(Player p) {
        ...
        while (isDefeated) {
                if (p.type == "human") {
                        System.out.println("That player is already out of the running!");
                }
        ...
        while (hasMaiden) {
                if (p.type == "human") {
                        System.out.println("That player just played HANDMAIDEN and is immune!");
                }
                pnum = p.whichPlayer(players.size());
...
private void guardAction(Player p) {
        ...
        if (hand.type == guess) {
                if (p.type == "human") {
                        System.out.println("Your guess was correct! Player #" + target.number + " no longer has a chance.");
                }
...
private void priestAction(Player p) {
        ...
        if (p.type == "human") {
                System.out.println("Their card is: " + hand + "\n");
        }
}
...
private void baronAction(Player p) {
        ...
        if (p1 > p2) {
                if (p.type == "human") {
                        System.out.println("You were victorious! Player #" + target.number + " no longer has a chance.");
                }
                defeated.add(target.number - 1);
                target.play(0, this);            // reveal their card by playing hand
        } else if (p2 > p1) {
                if (p.type == "human") {
                        System.out.println("You were defeated and no longer have a chance...");
                }
                defeated.add(p.number - 1);
                p.play(0, this);
        } else {
                if (p.type == "human") {
                        System.out.println("It was a tie! Nothing happens.");
                }
        }
}
...
private void princeAction(Player p) {
        Player target = validCheck(p);
        // Player must play Prince even if on themself
        if (target == null) {
                target = p;
        }
        Card c = deck.draw();
